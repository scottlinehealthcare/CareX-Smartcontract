...
contract Token {
    ...
    event TransferCarex(address indexed _from, address indexed _to, uint256 _value);
}
...
contract StandardToken is Token,Mortal,Pausable {
    ...
    function transfer(address _to, uint256 _value)public whenNotPaused returns (bool success) {
        ...
        require(_value>0);
         if (balances[msg.sender] >= _value) {
            ...
            TransferCarex(msg.sender, _to, _value);
            return true;
        } else { return false; }
    }
    ...
    function transferFrom(address _from, address _to, uint256 totalTokensToTransfer)public whenNotPaused returns (bool success) {
       ...
       if (balances[_from] >= totalTokensToTransfer&&allowance(_from,_to)>=totalTokensToTransfer) {
            ...
            TransferCarex(msg.sender, _to, totalTokensToTransfer);
            return true;
        } else { return false; }
    }
...
contract Care is StandardToken{
    ...
    function Care() public{
       totalSupply=10000000 * (10 ** decimals);  //Ten Million
       owner = msg.sender;
